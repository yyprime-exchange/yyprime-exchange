...
(async () => {
  ...
  await solanaClient.requestAirdrop(
    simulation.config.walletBalance,
    wallet.publicKey
  );
  ...
  for (const bot of simulation.bots) {
    ...
    const baseTokenAccount = await solanaClient.createTokenAccount(
    ...
    const quoteTokenAccount = await solanaClient.createTokenAccount(
    ...
    const openOrders: Keypair = Keypair.fromSecretKey(
      Buffer.from(bot.openOrdersPrivateKey, 'base64')
    );
    ...
    const transaction = new Transaction().add(
      SystemProgram.createAccount({
        ...
        lamports:
          await serumClient.connection.getMinimumBalanceForRentExemption(
            OpenOrders.getLayout(new PublicKey(simulation.config.serum.program))
              .span
          ),
        space: OpenOrders.getLayout(
          new PublicKey(simulation.config.serum.program)
        ).span,
        programId: new PublicKey(simulation.config.serum.program)
      })
    );
    await sendAndConfirmTransaction(serumClient.connection, transaction, [
      bot_wallet,
      openOrders
    ]);
  }
