...
(async () => {
  ...
  console.log(
    `  simulation.config.wallet = ${JSON.stringify(
      await solanaClient.connection.getParsedAccountInfo(
        new PublicKey(simulation.config.wallet)
      )
    )}`
  );
  console.log(
    `  Balance = ${
      (await solanaClient.connection.getBalance(
        new PublicKey(simulation.config.wallet)
      )) / LAMPORTS_PER_SOL
    } SOL`
  );
  ...
  for (const token of simulation.tokens) {
    ...
    const vault: Keypair = Keypair.fromSecretKey(
      Buffer.from(token.vaultPrivateKey, 'base64')
    );

    console.log(`TOKEN: ${token.symbol}`);
    console.log(
      `  mint = ${JSON.stringify(
        await solanaClient.connection.getParsedAccountInfo(mint.publicKey)
      )}`
    );
    console.log(
      `  vault = ${JSON.stringify(
        await solanaClient.connection.getParsedAccountInfo(vault.publicKey)
      )}`
    );
    console.log(
      `  Supply: ${await solanaClient.getMintSupply(
  ...
  for (const market of simulation.markets) {
    ...
    const requestQueueAccount = await solanaClient.connection.getAccountInfo(
    ...
    const eventQueueAccount = await solanaClient.connection.getAccountInfo(
    ...
    const asksAccount = await solanaClient.connection.getAccountInfo(
      new PublicKey(market.asks)
    );
    console.log(
      `  asks = ${JSON.stringify(
        Orderbook.decode(
          serumClient.getMarket(market.market),
          asksAccount!.data
        )
      )}`
    );

    const bidsAccount = await solanaClient.connection.getAccountInfo(
  ...
  for (const bot of simulation.bots) {
    const bot_wallet: Keypair = Keypair.fromSecretKey(
      Buffer.from(bot.walletPrivateKey, 'base64')
    );
    console.log(`BOT: ${bot.name}`);
    console.log(
      `  bot.wallet = ${JSON.stringify(
        await solanaClient.connection.getParsedAccountInfo(bot_wallet.publicKey)
      )}`
    );
    console.log(
      `  Balance = ${
        (await solanaClient.connection.getBalance(bot_wallet.publicKey)) /
        LAMPORTS_PER_SOL
      } SOL`
    );
    console.log(
      `  BaseTokenBalance = ${JSON.stringify(
        await solanaClient.getTokenBalance(
          await solanaClient.getAssociatedTokenAddress(
    ...
    console.log(
      `  QuoteTokenBalance = ${JSON.stringify(
        await solanaClient.getTokenBalance(
          await solanaClient.getAssociatedTokenAddress(
