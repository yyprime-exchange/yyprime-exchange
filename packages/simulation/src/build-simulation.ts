...
export class SimulationBuilder {
  ...
  public async build() {
    ...
    if (this.cluster === 'mainnet') {
      ...
      const serumMarkets = await SerumClient.query(
        connection,
        new PublicKey(config.serum.program)
      );
      const solanaTokens = await SolanaClient.query(
        connection,
        SOLANA_TOKENS.mainnet.map((token) => {
          return { symbol: token.symbol, mint: new PublicKey(token.mint) };
        })
      );
      ...
      const tokens = solanaTokens.map((token) => {
        ...
        return {
          ...
          supply:
            token.data.data.parsed.info.supply /
            this.pow10(token.data.data.parsed.info.decimals),
      ...
      serumMarkets.forEach((market) => {
        const symbol = `${mintSymbols.get(
          market.baseMint.toBase58()
        )}/${mintSymbols.get(market.quoteMint.toBase58())}`;
        if (
          !maxReferrerRebatesAccruedByMarket.has(symbol) ||
          market.referrerRebatesAccrued >
            maxReferrerRebatesAccruedByMarket.get(symbol).referrerRebatesAccrued
        ) {
      ...
      const markets = serumMarkets
        .filter((market) => {
          ...
          return (
            maxReferrerRebatesAccruedByMarket.get(symbol).ownAddress ===
              market.ownAddress &&
        ...
        .map((market) => {
          ...
          const baseToken = tokens.find((token) => {
            return token.symbol === baseSymbol;
          });
          ...
          return {
            ...
            basePrice: priceKeys.get(
              `${mintSymbols.get(market.baseMint.toBase58())}`
            ),
      ...
      const config_private = {
        ...
        walletPrivateKey: Buffer.from(
          simulationWalletKeypair.secretKey
        ).toString('base64'),
        walletBalance: 10
      };
      ...
      const tokens_private = this.tokens.map((symbol) => {
        const mainnetToken = simulationMainnet.tokens.find((mainnetToken) => {
          return mainnetToken.symbol === symbol;
        });
        ...
        return {
          ...
          vaultPrivateKey: Buffer.from(vaultKeypair.secretKey).toString(
            'base64'
          ),
      ...
      const markets_private = this.markets.map((symbol) => {
        const mainnetMarket = simulationMainnet.markets.find(
          (mainnetMarket) => {
            return mainnetMarket.symbol === symbol;
          }
        );
        ...
        return {
          ...
          marketPrivateKey: Buffer.from(marketKeypair.secretKey).toString(
            'base64'
          ),
          baseMint: baseToken.mint,
          baseVault: baseVaultKeypair.publicKey.toBase58(),
          baseVaultPrivateKey: Buffer.from(baseVaultKeypair.secretKey).toString(
            'base64'
          ),
          ...
          quoteVaultPrivateKey: Buffer.from(
            quoteVaultKeypair.secretKey
          ).toString('base64'),
          ...
          requestQueuePrivateKey: Buffer.from(
            requestQueueKeypair.secretKey
          ).toString('base64'),
          eventQueue: eventQueueKeypair.publicKey.toBase58(),
          eventQueuePrivateKey: Buffer.from(
            eventQueueKeypair.secretKey
          ).toString('base64'),
      ...
      const connection: Connection = new Connection(
        SERUM_PROGRAMS['mainnet'].url
      );

      const orders_private = await Promise.all(
        markets_private.map(async (market) => {
          let asks: [number, number, BN, BN][] = [];
          let bids: [number, number, BN, BN][] = [];

          const mainnetMarket = simulationMainnet.markets.find(
            (mainnetMarket) => {
              return mainnetMarket.symbol === market.symbol;
            }
          );
          const mainnetBaseToken = simulationMainnet.tokens.find(
            (mainnetToken) => {
              return mainnetToken.symbol === market.baseSymbol;
            }
          );
          const mainnetQuoteToken = simulationMainnet.tokens.find(
            (mainnetToken) => {
              return mainnetToken.symbol === market.quoteSymbol;
            }
          );
          ...
          if (mainnetMarket && mainnetBaseToken && mainnetQuoteToken) {
            asks = getPriceLevels(
              ...
              (await connection.getAccountInfo(
                new PublicKey(mainnetMarket.asks)
              ))!.data
            );
            bids = getPriceLevels(
              mainnetMarket,
              ...
              mainnetQuoteToken,
              (await connection.getAccountInfo(
                new PublicKey(mainnetMarket.bids)
              ))!.data
            );
          }
      ...
      const bots_private = this.bots.map((bot) => {
        ...
        const orders = orders_private.find(
          (order) => order.symbol === bot.symbol
        );
        assert(orders);
        ...
        return {
          ...
          walletPrivateKey: Buffer.from(walletKeypair.secretKey).toString(
            'base64'
          ),
          walletBalance: 10,
          openOrders: openOrdersKeypair.publicKey.toBase58(),
          openOrdersPrivateKey: Buffer.from(
            openOrdersKeypair.secretKey
          ).toString('base64')
        };
      });
...
(async () => {
  ...
  const simulationBuilder: SimulationBuilder = new SimulationBuilder(
    'localnet'
  );
  ...
  if (simulationBuilder.cluster === 'mainnet') {
    ...
    fs.writeFileSync(
      '../monitor/src/config/simulation-mainnet.json',
      JSON.stringify(simulation_public, null, 2)
    );
    fs.writeFileSync(
      'src/simulation-mainnet.json',
      JSON.stringify(simulation_public, null, 2)
    );
  } else {
    ...
    simulationBuilder.bot('BTC_mm_0', 'maker', 'BTC/USDC', {
      half_spread: 0.005
    });
    simulationBuilder.bot('ETH_mm_0', 'maker', 'ETH/USDC', {
      half_spread: 0.005
    });
    simulationBuilder.bot('SOL_mm_0', 'maker', 'SOL/USDC', {
      half_spread: 0.005
    });

    const [simulation_public, simulation_private] =
      await simulationBuilder.build();
    fs.writeFileSync(
      '../monitor/src/config/simulation.json',
      JSON.stringify(simulation_public, null, 2)
    );
    fs.writeFileSync(
      'src/simulation.json',
      JSON.stringify(simulation_private, null, 2)
    );
  }
})();
